...
**Maya v0.1.5 preview:** [Download the separate installable Maya ZIP](https://github.com/lyingtiger88/BDFR_AdvancedAutoRig/releases/download/maya-v0.1.5/BDFR_AdvancedAutoRig_Maya-0.1.5.zip) and follow the [Maya installation guide](maya_autorig/README.md). Extract the Maya ZIP, drag its `install.py` into Maya, restart, then load `bdfr_advanced_autorig.py` in Plug-in Manager. It includes a basic Analyze/Build/FBX interface, rigid skin binding, Simple wheel counts and Unreal/Unity axis presets. v0.1.5 exports an existing skinned Maya rig after reopening the tool or scene: select `BDFR_Root` and click Export, without building again. New rigs save export direction on the root; older rigs use their FRONT arrow to recover it. Multi-shell support and the Rigging shelf icon remain. Interactive Maya validation is pending. The v0.6.0 ZIP above installs Blender only.
