...
**Automatic first-pass vehicle and airplane rigging for Blender 4.2+.** Select a model, inspect detected parts, and build an armature with rigid weights and animation controls.

> **Status:** v0.6.0 preview. Detection uses object names, disconnected mesh islands, dimensions and position. It is heuristic geometry analysis, not machine-learning segmentation. Blender runtime validation on representative production files is still pending.
...
**Maya v0.1.4 preview:** [Download the separate installable Maya ZIP](https://github.com/lyingtiger88/BDFR_AdvancedAutoRig/releases/download/maya-v0.1.4/BDFR_AdvancedAutoRig_Maya-0.1.4.zip) and follow the [Maya installation guide](maya_autorig/README.md). Extract the Maya ZIP, drag its `install.py` into Maya, restart, then load `bdfr_advanced_autorig.py` in Plug-in Manager. It includes a basic Analyze/Build/FBX interface, rigid skin binding, Simple wheel counts and Unreal/Unity axis presets. v0.1.4 analyzes meshes with multiple disconnected shells, such as imported Body meshes: each mesh and its shells receive one rigid joint. The Analyze list displays shell counts; separate doors or other moving pieces for independent motion. The Rigging shelf icon and Maya 2027 loading fixes remain. Interactive Maya validation is pending. The v0.6.0 ZIP above installs Blender only.
...
- **Airplane** mode recognizes named propellers, landing wheels and separate gear struts, ailerons, elevators, rudders and flaps. In Simple mode, landing wheels and propellers rotate while other parts follow the fuselage. Advanced adds movable flight surfaces and gear struts; nearby landing wheels follow their strut. For a tricycle or taildragger layout, the lone nose or tail wheel steers automatically. Each landing wheel also has an **Auto / Yes / No** steering override.
...
Download [the add-on ZIP](https://github.com/lyingtiger88/BDFR_AdvancedAutoRig/releases/download/v0.6.0/BDFR_AdvancedAutoRig-0.6.0.zip). In Blender, open **Edit → Preferences → Add-ons → Install from Disk**, select the ZIP, and enable **BDFR Advanced AutoRig**. Open the 3D View sidebar with `N` and find the **Vehicle Rig** tab. Install the add-on ZIP, not GitHub's *Download ZIP* archive of the whole repository.

**Updating from an older build:** Disable and remove the existing BDFR Advanced AutoRig entry in Preferences → Add-ons, close Blender, then restart and install the v0.6.0 add-on ZIP. The top of the Vehicle Rig panel must read **BDFR Advanced AutoRig v0.6.0**. If you still see a Wheel roll slider and no Simple/Advanced switch, Blender is still loading an older installation. Installing another ZIP over it can leave the old module active until restart.
...
- Aircraft component recognition primarily uses mesh names or explicit **Mark** overrides. Wing control surface hinges and landing gear pivots are estimated from bounding boxes; adjust them in Edit Mode for a particular aircraft. Jet turbines, articulated multi-link landing gear, aerodynamic simulation and automatic flight paths are outside this first-pass rig. Gear struts without separate meshes stay fixed to the fuselage.
...
- A Blender 4.5 integration test checks airplanes in both rig modes (gear, propellers and control surfaces) as well as vehicle rigs, object-mode rig translation, suspension motion, bone counts, door/hood/trunk preview, wheel animation, pose deformation, imported hierarchy preservation and repair of an older rig. Test with a copy of a production vehicle before relying on it.
