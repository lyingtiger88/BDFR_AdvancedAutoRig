...
1. Download **[BDFR_AdvancedAutoRig_Maya-0.1.4.zip](https://github.com/lyingtiger88/BDFR_AdvancedAutoRig/releases/download/maya-v0.1.4/BDFR_AdvancedAutoRig_Maya-0.1.4.zip)**. Extract the **entire** archive to a temporary folder.
...
**Upgrading:** unload `bdfr_advanced_autorig.py`, run the installer from the v0.1.4 ZIP, restart Maya, then enable it in Plug-in Manager. v0.1.4 accepts multi-shell mesh objects as rigid parts, retaining the earlier Maya 2027 loading fixes and the Rigging shelf button. The shelf button is removed on plug-in unload and created again when loaded. Automated tests cover plug-in registration, execution without `__file__`, multi-shell analysis and rigid binding; interactive Maya 2027 validation is still pending.
